...
//
// The order matters and is the point: [Resolve] settles every name and every
// option and rejects whatever is left unclaimed, then the schema is checked
// against each backend's capabilities, and only then is anything rendered. A
// user who misspelled an option finds out before a file exists, not from output
// that quietly used a default.
func Run(s *schema.Schema, cfg *Config, r *Registry, options ...RunOption) (*Output, error) {
	...
	for _, p := range plan {
		...
		if p.Backend != nil {
			...
			if len(refused) > 0 {
				return nil, fmt.Errorf("%s: %w", p.Config.Label(), refused.Err())
			}
			if len(accepted) > 0 {
				fmt.Fprintf(opts.warn, "calque: %s: %v\n", p.Config.Label(), accepted.Err())
			}

			var err error
			lowered, err = p.Backend.Lower(s)
			if err != nil {
				return nil, fmt.Errorf("%s: lower for %s: %w", p.Config.Label(), p.Backend.Name(), err)
			}
		...
		g := newGenerator(s, lowered, p.Backend, p.Config, &diags)
		g.req, g.files, g.warn = opts.req, opts.files, opts.warn
		g.progress, g.label = opts.progress, p.Config.Label()
		files, err := p.Target.Emit(g)
		if err != nil {
			return nil, fmt.Errorf("%s: %w", p.Config.Label(), err)
		}
		for _, f := range files {
			if err := out.Add(p.Config.Label(), g.Out(f.Name), f.Body); err != nil {
				return nil, err
			}
		}
		opts.progress.TargetDone(p.Config.Label(), backendName(p.Backend), len(files))
	}
